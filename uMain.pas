...
uses
  ...
  FMX.Memo, Windows, Messages, FireDAC.Phys.PG, FireDAC.Phys.PGDef;
...
begin
  ...
  begin
    ...
    begin
      ...
      SQL.Text := 'Select * From d_products where id = ' + IntToStr(ProductID);
      Open;
      ProductQuantity := StrToFloat(StringGrid1.Cells[2, i]);
      ProductPriceExVAT := FieldByName('price_with_vat').Value /
        (1 + FieldByName('vat_ratio').Value);
      ProductAmount := ProductQuantity * FieldByName('price_with_vat').Value;
      Close;
    end;
...
begin
  ...
  begin
    ShowMessage('Please check the paid amount against the invoice amount!');
    Exit;
  end;
  ...
  begin
    ...
    begin
      Open;
      Insert;
      FieldByName('customer_id').Value := 0;
      FieldByName('employee_id').Value := 0;
      FieldByName('order_date_time').Value := Now;
      FieldByName('order_amount').Value := edSubTotal.Text;
      FieldByName('vat_amount').Value := edTax.Text;
      FieldByName('payment_method').Value := edPaymentMethod.Text;
      Post;
    end;
    ...
    begin
      ...
      begin
        Open;
        Insert;
        FieldByName('order_id').Value := OrderID;
        FieldByName('product_id').Value := ProductID;
        FieldByName('price_with_vat').Value := UnitPrice;
        FieldByName('quantity').Value := Quantity;
        FieldByName('discount').Value := 0;
        FieldByName('vat_ratio').Value := Vat / 100;
        FieldByName('line_value').Value := LineValue;
        Post;
      end;
    end;
    // Reset screen
...
begin
  ...
  begin
    Close;
    SQL.Text := 'Select * From d_products Where touch_flag is true';
    Open;
    ...
    begin
      ...
      StringGrid2.Cells[1, i] := FieldByName('product_name').AsString;
      StringGrid2.Cells[2, i] := FieldByName('price_with_vat').AsString;
      i := i + 1;
      Next;
    end;
    Close;
...
begin
  Try
    if edTotal.Text.ToSingle <= 0 then
      Exit;
  ...
  Except
    on E: Exception do
      if POS('is not a valid', E.Message) <> 0 then
...
begin
  Try
    if edTotal.Text.ToSingle <= 0 then
      Exit;
    edPaid.Text := edTotal.Text;
    edChange.Text := FormatFloat('#0.00',
      (edTotal.Text.ToSingle - edPaid.Text.ToSingle));
    edPaymentMethod.Text := 'CRT';
  Except
    on E: Exception do
...
begin
  ...
  Qty := FMX.Dialogs.InputBox(StringGrid1.Cells[1, StringGrid1.Row],
    'Enter Quantity for ' + StringGrid1.Cells[1, StringGrid1.Row],
...
begin
  ...
  btnAddClick(Self);
  StringGrid1.SelectCell(0, GetFilledRows - 1);
  StringGrid1CellDblClick(StringGrid1.Columns[0], GetFilledRows - 1);
end;
...
begin
  ...
  with qryProduct do
  begin
    SQL.Text := 'Select * From d_products Where barcode = ' +
    ...
    begin
      ProductID := FieldByName('id').Value;
      ProductName := FieldByName('product_name').Value;
      ProductQuantity := 1;
      ProductPrice := FieldByName('price_with_vat').Value;
      ProductVAT := FieldByName('vat_ratio').Value;
